...
class AnthropicBlogSpider(scrapy.Spider):
    """Anthropic news — HTML scraping (no RSS feed available)."""
    name = 'anthropic'
    ...
    def parse(self, response):
        ...
        article_links = response.css('a[href*="/news/"], a[href*="/research/"]')
        seen = set()
        for link in article_links:
            href = link.css('a::attr(href)').get('') or link.attrib.get('href', '')
            if not href or not href.startswith('/news/') and not href.startswith('/research/'):
                continue
    ...
    def parse_article(self, response):
        ...
        if not title:
            title = response.css('[class*="title"] h1::text').get('')
        if not title:
            return
        ...
        # Try to extract date
        date_text = response.css('time::text, [class*="date"]::text').get('')
