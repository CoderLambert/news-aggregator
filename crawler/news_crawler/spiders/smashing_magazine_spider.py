...
class SmashingMagazineSpider(scrapy.Spider):
    name = 'smashing_magazine'
    allowed_domains = ['smashingmagazine.com']
    start_urls = ['https://www.smashingmagazine.com/feed/']
    ...
    def parse(self, response):
        ...
        for item in response.css('item'):
            ...
            news['source_name'] = 'Smashing Magazine'
    ...
    def parse_article(self, response):
        ...
        blocks = response.css('.article-content p::text').getall()
        if not blocks:
        ...
        author = response.css('.article-author__name::text, .author::text').get() or ''
        ...
        news['cover_image'] = response.css('figure img::attr(src), .article-content img::attr(src)').get() or ''
        yield news
