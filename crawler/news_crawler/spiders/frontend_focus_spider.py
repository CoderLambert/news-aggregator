...
class FrontendFocusSpider(scrapy.Spider):
    """Frontend Focus — curated weekly frontend newsletter roundup."""
    name = 'frontend_focus'
    allowed_domains = ['frontendfoc.us']
    start_urls = ['https://frontendfoc.us/rss']
    ...
    def parse(self, response):
        ...
        for item in response.css('item'):
            ...
            news['author'] = author.strip() if author else 'Frontend Focus'
            news['publish_time'] = publish_time
            news['source_name'] = 'Frontend Focus'
            news['category_name'] = classify(title.strip(), description)
            news['url'] = url
            news['cover_image'] = item.css('enclosure::attr(url)').get('') or ''
            yield news

            # Frontend Focus links to external articles; follow for full content
    ...
    def parse_article(self, response):
        ...
        news['author'] = 'Frontend Focus'
        news['publish_time'] = datetime.now()
        news['source_name'] = 'Frontend Focus'
        news['category_name'] = classify(title.strip(), content)
        news['url'] = response.url
        news['cover_image'] = response.css('article img::attr(src), .content img::attr(src)').get() or ''
        yield news
