...
class ArsTechnicaAiSpider(scrapy.Spider):
    name = 'ars_technica_ai'
    allowed_domains = ['arstechnica.com']
    start_urls = ['https://arstechnica.com/tag/artificial-intelligence/feed/']
    ...
    def parse(self, response):
        ...
        for item in response.css('item'):
            ...
            news['source_name'] = 'Ars Technica AI'
    ...
    def parse_article(self, response):
        ...
        blocks = response.css('article p::text, .article-body p::text').getall()
        if not blocks:
        ...
        author = response.css('.byline a::text, .author::text').get() or ''
